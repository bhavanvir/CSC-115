...
public class ExpenseCalculator {
     ...
     */
    // TODO...
	public static double calcHotel(double rate, int nights){
		double cost = 0;
        double tax = 0;
	...
	public static double calcFood(int days){
		...
		if(days >= DAYS_PER_WEEK){
			expense = (double)days/(double)DAYS_PER_WEEK * (double)WEEKLY_FOOD_RATE;
		}else{
			expense = (double)days * (double)DAILY_FOOD_RATE;
		}
		
		if((double)days/(double)DAYS_PER_WEEK > 4){
	...
	public static double calcTotalExpense(double[] data){
		...
		mileage = calcMileage(data[KMS_DRIVE_INDEX]);
		hotel = calcHotel(data[HOTEL_COST_INDEX], (int) (data[NUM_DAYS_INDEX])-1);
		food = calcFood((int) data[NUM_DAYS_INDEX]);
	...
	public static int[] calcAllExpenses(double[][] data){
		...
		for(int i = 0; i < data.length; i++){
			expense_results[i] = (int)calcTotalExpense(data[i]);
		}
